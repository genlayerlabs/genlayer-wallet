...
import { BytesLike, decodeRlp, getBytes, Interface } from 'ethers';
 ...
 */
export function parseGenLayerTransaction(data: string): { contractAddress: string; methodName: string } {
  try {
    const contractInterface = new Interface(chains.localnet.consensusMainContract?.abi as any);
 ...
 */
export function generateStorageKey(contractAddress: string, methodSelector: string): string {
 ...
 */
export function getTransactionStorageKey(transaction: { to?: string; data?: string }): string {
  const parsed = parseGenLayerTransaction(transaction.data || '');
  const storageKey = generateStorageKey(parsed.contractAddress, parsed.methodName);
